...
class RealRolesDAOFacade : RolesDAOFacade {
    override suspend fun getRoles() = dbQuery {
        RoleEntity.all().map { it.toRole() }
    }

    override suspend fun getIdByRole(role: Role) = dbQuery {
        RoleEntity.find { (Roles.name.upperCase() eq role.name.uppercase()) }
            .limit(1)
            .first()
            .id.value
    }

    override suspend fun getRoleById(id: Int) = dbQuery {
    ...
    private fun RoleEntity.toRole(): Role {
        return Role.entries.first { it.name.uppercase() == name.uppercase() }
    }
}